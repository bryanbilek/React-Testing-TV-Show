...
test("Episodes component renders the list of episodes", () => {
    //test that confirms Episodes has no data
    const { queryAllByTestId, rerender } = render(<Episodes episodes={[]} />);
    expect(queryAllByTestId(/episodes-list/i)).toHaveLength(0);

    //test that confirms Episodes got data as props
    rerender(<Episodes episodes={mockEpisodesData} />);
    expect(queryAllByTestId(/episodes-list/i)).toHaveLength(2);
});
